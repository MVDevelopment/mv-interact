...
RegisterNUICallback("NUIFocusOff", function(data, cb)
    ...
    TriggerEvent('mv-interact:closeMenu')
end)

RegisterNetEvent('mv-interact:closeMenu')
AddEventHandler('mv-interact:closeMenu', function()
    FreezeEntityPosition(PlayerPedId(), false)
...
RegisterNetEvent('mv-interact:enableSlider')
AddEventHandler('mv-interact:enableSlider', function()
    FreezeEntityPosition(PlayerPedId(), false)
    SendNUIMessage({
        action = "enableSlider"
    })
...
RegisterNetEvent('mv-interact:simulateClose')
AddEventHandler('mv-interact:simulateClose', function()
    FreezeEntityPosition(PlayerPedId(), false)
    SendNUIMessage({
...
RegisterNetEvent('mv-interact:generateMenu')
AddEventHandler('mv-interact:generateMenu', function(menu, title, cancel, keepOpen, options)
    FreezeEntityPosition(PlayerPedId(), false)
...
RegisterNetEvent('mv-interact:generateForm')
AddEventHandler('mv-interact:generateForm', function(trigger, triggertype, form, title, center, keepOpen, width, cancel, opt)
...
RegisterNetEvent('mv-interact:generateBlips')
AddEventHandler('mv-interact:generateBlips', function(triggers, cancel)
    FreezeEntityPosition(PlayerPedId(), false)
...
RegisterNetEvent('mv-interact:generateBlipColor')
AddEventHandler('mv-interact:generateBlipColor', function(triggers, cancel)
    FreezeEntityPosition(PlayerPedId(), false)
    if triggers ~= nil then
        SetNuiFocus(true, true)
        SendNUIMessage({
...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustPressed(0, 38) then
            TriggerEvent('mv-interact:generateBlipColor', { ['trigger'] = "test", ['type'] = "client" }, {{['trigger'] = 'test', ['method'] = "client"}})
        end
    end
end)
]]
local open = false
RegisterNetEvent('mv-interact:generateColorPicker')
AddEventHandler('mv-interact:generateColorPicker', function(triggers, title, fdata, options, cancel)    
    FreezeEntityPosition(PlayerPedId(), false)
...
RegisterNetEvent('mv-interact:generateSkinChangeMenu')
AddEventHandler('mv-interact:generateSkinChangeMenu', function(title, sliders, options)
    FreezeEntityPosition(PlayerPedId(), false)
    SetNuiFocus(true, true)
    SendNUIMessage({
        title = title,
...
RegisterNetEvent('mv-interact:generateSlider')
AddEventHandler('mv-interact:generateSlider', function(options, trigger, triggertype, title, desc, preload, cancel)
    FreezeEntityPosition(PlayerPedId(), false)
    if options ~= nil then
        SetNuiFocus(true, true)
        SendNUIMessage({
...
RegisterNetEvent('mv-interact:test')
AddEventHandler('mv-interact:test', function(result)
    print('Test 1 Trigger')
    tprint(result)
end)

RegisterNetEvent('mv-interact:test2')
AddEventHandler('mv-interact:test2', function(result)
...
function openForm()
    local options = {  
                    { ['label'] = "Aldi A3 testing a long ass name be", ['data'] = { ['model'] = "aa3", ['price'] = 1600, ['trigger'] = "mv-interact:test2", ['triggerType'] = "client"} },
                    { ['label'] = "Aldi A2", ['data'] = { ['model'] = "aa2", ['price'] = 163550, ['trigger'] = "mv-interact:test2", ['triggerType'] = "client"} },
                    { ['label'] = "Aldi A4", ['data'] = { ['model'] = "aa4", ['price'] = 16350, ['trigger'] = "mv-interact:test2", ['triggerType'] = "client"} },
                }
    TriggerEvent('mv-interact:generateSlider', options, 'mv-interact:test', 'client', "Slider Title", "Slider Description can go here including any <span class='text-success'>HTML Formatting</span>", {vehicle = true})
end
